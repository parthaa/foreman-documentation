...
For example, you have Products and repositories that your IT department validates on {ProjectServer} in the Library, and you want to copy all Products and repositories in that organization to other {ProjectServer}s.
...
[[img-disconnected]]
ifdef::satellite[]
image::Disconnected.png[title="The {ProjectNameX} ISS use cases", alt="The {ProjectNameX} ISS use cases"]
endif::[]
...
.. For existing repos using *On Demand* change their download policy on the repository details page to *Immediate* and resync
...
. Synchronize the enabled content.
. On updates republish the desired Content Views (see xref:Managing_Content_Views[].)
. For the first export do a `complete version` export on the desired Content View version.(see xref:Using_ISS-Exporting-a-Content-View-Version[].)  This will generate content archives that could be imported into one or more disconnected {ProjectServer}s.
. Export all future updates in the connected {ProjectServer}s incrementally (see xref:Using_ISS-Exporting-a-Content-View-Version-Incremental[].) This will generate leaner content archives with only changes from the previous set of updates. So for example if your Content View had a new repository this exported content archive will only contain the changes.
...
If you are exporting content to several disconnected {ProjectServer}s then using a `--destination-server` option can be very useful  (see xref:Using_ISS-Destination-Server[].)  This will indicate to the exporting {ProjectServer} and provide a way to organize/book keep on what versions got exported to a given destination.
...
You are exporting and importing content across several {ProjectServer}s.
If you have to keep track of which server imports what content and what can and cannot be incrementally updated, things can get tedious.
This is where the `--destination-server` option becomes useful.
...
=== Exporting your Library Environment
...
=== Importing into your Library Environment
...
The import and export functionality can be used mainly in the following scenarios:
...
. Set the`Default Red Hat Repository download policy` to *Immediate* (see xref:Importing_Content-Configuring_Download_Policies[].)
. Enable the content that you want to use (refer to xref:Importing_Content-Selecting_Red_Hat_Repositories_to_Synchronize[].)
. Synchronize the enabled content.
. For the first export, perform a `complete Library` export so that all the synchronized content are exported.
This  generates content archives that you can later import into one or more disconnected {ProjectServer}s.
